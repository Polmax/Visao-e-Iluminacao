...
uniform float edge_thres;
uniform float edge_thres2;
...
float[HueLevCount] HueLevels = float[] (0.0,60.0,120.0,240.0,300.0,360.0);
...
{
    return texture(texDay,coords + vec2(dx, dy));
}

// returns pixel color
float IsEdge(vec2 coords)
{
    float dxtex = 1.0 /float(textureSize(texDay,0)) ;
    float dytex = 1.0 /float(textureSize(texDay,0));
    float pix[9];
    int k = -1;
    float delta;
...
void main() {
    ...
    // get texture color
    vec3 colorOrg = texture(texDay, DataIn.texCoord).rgb;
    ...
    vec4 eColor = vec4(vRGB.x, vRGB.y, vRGB.z, 1);
